...
hacking_function_4(PG_FUNCTION_ARGS)
{
	...
	/* get tuple descriptor (list of attributes, see pg_attribute) */
	tdesc = RelationGetDescr(rel);

	/* */
	for (int i = 0; i < tdesc->natts; i++)
	{
		FormData_pg_attribute *attr = TupleDescAttr(tdesc, i);

		elog(WARNING, "%d attnum %d typlen %d typbyval %d", i,
			 attr->attnum, attr->attlen, attr->attbyval);
